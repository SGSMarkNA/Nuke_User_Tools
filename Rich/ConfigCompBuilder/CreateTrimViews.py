...
####################################################
class CreateTrimViews(nukescripts.PythonPanel):
	...
	##################################
	#### CLEANUP VIEWS...  (this code borrowed from TrimViewGenerator)
	##################################
	...
	def _createTrimsInputPanel(self):
	...
	def _createColorsInputPanel(self):
		''' Get a list of colors from the user and sanitize the input - removing spaces and illegal characters. Create views from the trim names + colors entered by the user...'''
	...
	def _createTrimColorViews(self):
		''''''
		# Check for no input from user.
	...
	def _createTrimViewSection(self):
		...
		if not self.trims_to_build or not self.colors_to_build:
			...
			center = self._nodeList_center(self.List_of_OneView_Nodes)
			#print 'center----->', center
			...
			for x in self.List_of_OneView_Nodes:
				while counter >0:
					...
					end_index += num_colors
					counter -= 1
					center = self._nodeList_center(self.Common_OneView_Nodes)
...
def start():
	...
	x._createTrimsInputPanel()

	x._createColorsInputPanel()

	x._createTrimColorViews()

	x._storeTrimColorViews()

	x._createTrimViewSection()
